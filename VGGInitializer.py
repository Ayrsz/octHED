...
class OctaveVGGInitializer(AbstractVGGInitializer):
    def load(self, net : nn.DataParallel, device : str):
        ...
        hed_blocks = [
            [layer for (name, layer) in net.named_children() if 'conv1' in name],
            [layer for (name, layer) in net.named_children() if 'conv2' in name],
            [layer for (name, layer) in net.named_children() if 'conv3' in name],
            [layer for (name, layer) in net.named_children() if 'conv4' in name],
            [layer for (name, layer) in net.named_children() if 'conv5' in name]
        ]
    ...
    def _transfer_octave(self, oct_layer, vgg_conv):
        ...
        if oc.conv_h2h is not None:
            oc.conv_h2h.weight.data.copy_(vgg_conv.weight.data[:oc.conv_h2h.out_channels, :oc.conv_h2h.in_channels])
            oc.conv_h2h.bias.data.copy_(vgg_conv.bias.data[:oc.conv_h2h.out_channels])
